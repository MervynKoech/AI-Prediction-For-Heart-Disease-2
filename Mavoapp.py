...
def load_artifacts():
    ...
    # --- FIX for Error 1 & 3: Force numpy output to bypass pandas dtype bugs in scikit-learn 1.4+ ---
    try:
        ...
        pass # Fails gracefully if using an older scikit-learn version
...
# --- Page 2: Model Performance ---
    ...
    with col2:
        ...
        # --- FIX for Error 2: Flatten arrays to 1D numpy arrays to prevent TypeErrors ---
        y_test_vals = np.ravel(test_artifacts["y_test"])
        y_prob_rf_vals = np.ravel(test_artifacts["y_prob_rf"])
        y_prob_xgb_vals = np.ravel(test_artifacts["y_prob_xgb"])
        
        RocCurveDisplay.from_predictions(y_test_vals, y_prob_rf_vals, name="Random Forest", ax=ax_roc, color="blue")
        RocCurveDisplay.from_predictions(y_test_vals, y_prob_xgb_vals, name="XGBoost", ax=ax_roc, color="green")
    ...
    # --- FIX for Error 2: Flatten predictions as well ---
    y_pred_rf_vals = np.ravel(test_artifacts["y_pred_rf"])
    y_pred_xgb_vals = np.ravel(test_artifacts["y_pred_xgb"])
    
    if model_cm == "Random Forest":
        ConfusionMatrixDisplay.from_predictions(y_test_vals, y_pred_rf_vals, ax=ax_cm, cmap="Blues")
    else:
        ConfusionMatrixDisplay.from_predictions(y_test_vals, y_pred_xgb_vals, ax=ax_cm, cmap="Greens")
    st.pyplot(fig_cm)
...
# --- Page 4: Explainable AI (XAI) ---
    ...
    if 'latest_input' in st.session_state:
        ...
        # --- FIX: Robustly extract positive class values and base value ---
        if isinstance(shap_values, list):
            shap_values_pos = shap_values[1][0] # Extract positive class, then first sample
            raw_base_val = explainer.expected_value
        elif isinstance(shap_values, np.ndarray) and len(shap_values.shape) == 3:
            shap_values_pos = shap_values[0, :, 1] # 3D array: sample 0, all features, class 1
            raw_base_val = explainer.expected_value
        else:
            shap_values_pos = shap_values[0] # 2D array: just first sample
            raw_base_val = explainer.expected_value
            
        # Safely convert raw_base_val to a scalar float
        if isinstance(raw_base_val, (np.ndarray, list)):
            arr = np.array(raw_base_val)
            if arr.size > 1:
                base_val = float(arr[1]) # Index 1 is the positive class
            else:
    ...
    # Create an array of base_values matching the number of test samples
